...
/***/ function(module, exports, __webpack_require__) {
	...
	var _MainWeather = __webpack_require__(161);
	...
	var App = _react2.default.createClass({
		...
		getCoords: function getCoords() {
			...
			return new Promise(function (resolve, reject) {
				...
				navigator.geolocation.getCurrentPosition(function (pos) {
					...
					_this.setState({ coords: pos.coords });
					return resolve();
				}, function (err) {
					reject(err);
				});
		...
		convertTemp: function convertTemp(temp) {
		...
		toggleFormat: function toggleFormat() {
			...
			var hours = this.state.hours.map(function (elem) {
				var obj = elem;
				obj.temp = _this2.convertTemp(obj.temp);
				return obj;
			});
			var days = this.state.days.map(function (elem) {
				var obj = elem;
				obj.tempMin = _this2.convertTemp(obj.tempMin);
				obj.tempMax = _this2.convertTemp(obj.tempMax);
				return obj;
			});
			...
			this.setState({ fahrenheit: !this.state.fahrenheit, current: current, days: days, hours: hours });
	    ...
	    * use. 
	    */
				var _state$coords = _this3.state.coords;
				var long = _state$coords.longitude;
				var lat = _state$coords.latitude;
				//see if it's already in the localStorage to avoid server call
	
				if (localStorage.getItem('addr')) {
					_this3.setState({ location: localStorage.getItem('addr') });
				} else {
					_ajax2.default.get('/l?long=' + long + '&lat=' + lat, function (res) {
						localStorage.setItem('addr', res.addr);
						_this3.setState({ location: res.addr });
					});
				}
				if (localStorage.getItem('weather') && new Date().getTime() - localStorage.getItem('lastRefresh') < 300000) {
					var _JSON$parse = JSON.parse(localStorage.getItem('weather'));
	
					var current = _JSON$parse.current;
					var hours = _JSON$parse.hours;
					var days = _JSON$parse.days;
	
					_this3.setState({ current: current, hours: hours, days: days });
				} else {
					_ajax2.default.get('/w?long=' + long + '&lat=' + lat, function (res) {
						localStorage.setItem('weather', JSON.stringify(res));
						localStorage.setItem('lastRefresh', new Date().getTime());
						_this3.setState({ current: res.current, hours: res.hours, days: res.days });
					});
				}
			}).catch(function (err) {
				console.error(err);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _Current = __webpack_require__(162);
	...
	var MainWeather = _react2.default.createClass({
	  ...
	  render: function render() {
	    ...
	    return _react2.default.createElement(
	      ...
	      function () {
	        switch (_this.props.tab) {
	          case 1:
	            return _react2.default.createElement(_Current2.default, {
	              toggleFormat: _this.props.toggleFormat,
	              weather: _this.props.current,
	...
	var Current = _react2.default.createClass({
	  ...
	  render: function render() {
	    return _react2.default.createElement(
	      ...
	      _react2.default.createElement(
	        'div',
	        { className: 'current-display' },
	        _react2.default.createElement(
	          'h4',
	          { onClick: this.props.toggleFormat },
	          ' ',
	          this.props.weather.temp + this.props.format,
	          ' '
	        ),
	        _react2.default.createElement('i', { className: _utils.table[this.props.weather.icon] })
	      )
	    );
	  }
	});
...
/***/ function(module, exports, __webpack_require__) {
	...
	var SwitchTab = _react2.default.createClass({
		...
		render: function render() {
			return _react2.default.createElement(
				...
				_react2.default.createElement('input', { onClick: this.toggleTab, className: this.props.tab === 1 ? 'tab-selected' : '', type: 'button', 'data-index': '1', value: 'Current' }),
				_react2.default.createElement('input', { onClick: this.toggleTab, className: this.props.tab === 2 ? 'tab-selected' : '', type: 'button', 'data-index': '2', value: 'Forecast' }),
				_react2.default.createElement('input', { onClick: this.toggleTab, className: this.props.tab === 3 ? 'tab-selected' : '', type: 'button', 'data-index': '3', value: 'Visualize' })
			);
		}
	});
